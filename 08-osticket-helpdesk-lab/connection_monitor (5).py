...
# =====================================================================
#  ALERT STORAGE — keeps last 200 alerts for the dashboard
# =====================================================================
...
def store_alert(alert_type, details):
    """Saves every alert so the dashboard can display them."""
    with alert_history_lock:
        ...
        # Keep only the last 200 alerts in memory
...
def trigger_alert(alert_type, details):
    """Master alert — logs, stores for dashboard, emails, desktop."""
...
SUSPICIOUS_PATTERNS = [
    ...
    ("; ls ",             "COMMAND INJECTION",   "ls command injection detected"),
    ("; cat /etc/passwd", "COMMAND INJECTION",   "Attempt to read /etc/passwd detected"),
    ("; whoami",          "COMMAND INJECTION",   "whoami command injection detected"),
    ("| nc ",             "COMMAND INJECTION",   "Netcat pipe detected - possible reverse shell"),
    ("bash -i",           "COMMAND INJECTION",   "Bash interactive shell attempt detected"),
    ("/bin/sh",           "COMMAND INJECTION",   "Shell execution attempt detected"),
...
#  A simple web server that shows all alerts in your browser
#  Open: http://192.168.126.131:9999
# =====================================================================
...
<head>
  ...
  <style>
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    ...
    .stat-box .count {{ font-size: 28px; font-weight: bold; color: #f85149; }}
    .stat-box .label {{ font-size: 12px; color: #8b949e; margin-top: 4px; }}
    .stat-box.safe .count {{ color: #3fb950; }}
    .stat-box.warn .count {{ color: #d29922; }}
    ...
    .badge.orange {{ background: #2d1f0a; color: #d29922; border: 1px solid #d29922; }}
    .badge.blue   {{ background: #0d2044; color: #58a6ff; border: 1px solid #58a6ff; }}
    .badge.green  {{ background: #0d2818; color: #3fb950; border: 1px solid #3fb950; }}
...
<body>
  ...
  <div class="stats">
    ...
    <div class="stat-box">
      <div class="count">{port_scan_count}</div>
      <div class="label">Port Scans</div>
    </div>
    <div class="stat-box">
      <div class="count">{arp_count}</div>
      <div class="label">ARP Spoof</div>
    </div>
    <div class="stat-box">
      <div class="count">{ssl_count}</div>
      <div class="label">SSL Strip</div>
    </div>
    <div class="stat-box">
      <div class="count">{injection_count}</div>
      <div class="label">Injections</div>
    </div>
    <div class="stat-box warn">
      <div class="count">{connection_count}</div>
...
def get_badge(alert_type):
    """Returns a coloured badge based on alert type."""
    alert_type = alert_type.upper()
    if any(x in alert_type for x in ["INJECTION", "SPOOF", "WEBSHELL", "XSS", "TRAVERSAL"]):
        return f'<span class="badge red">{alert_type}</span>'
    elif any(x in alert_type for x in ["SCAN", "SSL", "STRIP", "SUSPICIOUS"]):
        return f'<span class="badge orange">{alert_type}</span>'
    elif "CONNECTION" in alert_type:
        return f'<span class="badge blue">{alert_type}</span>'
    else:
        return f'<span class="badge green">{alert_type}</span>'


class DashboardHandler(BaseHTTPRequestHandler):
    """Handles HTTP requests to the dashboard."""
    ...
    def do_GET(self):
        if self.path == "/" or self.path == "/dashboard":
            ...
            with alert_history_lock:
                alerts = list(reversed(alert_history))  # Newest first
            ...
            # JSON API endpoint for raw alert data
            self.send_response(200)
    ...
    def log_message(self, format, *args):
        # Suppress default HTTP server logs to keep terminal clean
        pass


def start_dashboard():
    """Starts the web dashboard on DASHBOARD_PORT."""
    try:
...
# =====================================================================
#  PACKET SNIFFER — runs all detection modules
# =====================================================================
